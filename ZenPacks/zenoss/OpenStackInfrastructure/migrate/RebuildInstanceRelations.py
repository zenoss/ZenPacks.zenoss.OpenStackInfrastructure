...
class RebuildInstanceRelations(ZenPackMigration):

    """Rebuilds relations on all Instance objects.

    This is necessary anytime new relations are added to Instance.
    ...
    you add a new relationship to Instance. No other changes to this
    ...
    def migrate(self, pack):
        results = ICatalogTool(pack.dmd.Devices).search(Instance)
    ...
    def updateRelations(self, device):
        for relname in (x[0] for x in Instance._relations):
