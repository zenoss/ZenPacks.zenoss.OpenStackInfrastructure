...
class TestImpact(zenpacklib.TestCase):
    ...
    @require_zenpack('ZenPacks.zenoss.Impact')
    def test_Host(self):
        hosts = self.endpoint().getDeviceComponents(type='OpenStackInfrastructureHost')
    ...
    def test_NovaService(self):
        nova_services = self.endpoint().getDeviceComponents(type='OpenStackInfrastructureNovaService')
        ...
        for nova_service in nova_services:
            ...
            self.assertEquals(zone.meta_type, "OpenStackInfrastructureAvailabilityZone")
    ...
    def test_Hypervisor(self):
        hypervisors = self.endpoint().getDeviceComponents(type='OpenStackInfrastructureHypervisor')
    ...
    def test_Cell(self):
        cells = self.endpoint().getDeviceComponents(type='OpenStackInfrastructureCell')
        ...
        for cell in cells:
            ...
            if org.meta_type == 'OpenStackInfrastructureAvailabilityZone':
                zone = org
                self.assertTrue(zone.id in impacts,
                                msg="Cell %s impacts its zone %s" % (cell.id, zone.id))
            elif org.meta_type == 'OpenStackInfrastructureCell':
                parentcell = org
            ...
            for childOrg in cell.childOrgs():
                if childOrg.meta_type == 'OpenStackInfrastructureCell':
    ...
    def test_AvailabilityZone(self):
        zones = self.endpoint().getDeviceComponents(type='OpenStackInfrastructureAvailabilityZone')
    ...
    def test_Region(self):
        ...
        for childOrg in region.childOrgs():
            if childOrg.meta_type == 'OpenStackInfrastructureAvailabilityZone':
                self.assertTrue(childOrg.id in impacted_by,
    ...
    @require_zenpack('ZenPacks.zenoss.Impact')
    def test_Vnic(self):
        vnics = self.endpoint().getDeviceComponents(type='OpenStackInfrastructureVnic')
